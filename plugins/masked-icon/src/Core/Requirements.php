...
final class Requirements {
	...
	public static function met(): bool {
		if ( version_compare( PHP_VERSION, MASKED_ICON_MIN_PHP, '<' ) ) {
			self::$reason = sprintf(
				/* translators: 1: required PHP version, 2: current PHP version */
				'Masked Icon requires PHP %1$s or newer. This server runs %2$s.',
				MASKED_ICON_MIN_PHP,
				PHP_VERSION
			);
		...
		if ( version_compare( (string) get_bloginfo( 'version' ), MASKED_ICON_MIN_WP, '<' ) ) {
			self::$reason = sprintf(
				/* translators: 1: required WordPress version, 2: current WordPress version */
				'Masked Icon requires WordPress %1$s or newer. This site runs %2$s.',
				MASKED_ICON_MIN_WP,
