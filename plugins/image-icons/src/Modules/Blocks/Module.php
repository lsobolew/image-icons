...
final class Module implements ModuleContract {
	...
	public function pass_mask_formats(): void {
		$handle = generate_block_asset_handle( 'image-icons/icon', 'editorScript' );
	 ...
	 * button - or a paragraph with an inline icon - and no Image Icons block would get the markup
	 * without the CSS.
	 *
	...
	private function enqueue_host_block_styles(): void {
		...
		foreach ( self::HOST_BLOCKS as $block ) {
			wp_enqueue_block_style(
				$block,
				array(
					'handle' => 'image-icons-inline',
					'src'    => IMAGE_ICONS_URL . $relative,
					'path'   => $path,
	...
	public function block_directories(): array {
		$build = IMAGE_ICONS_DIR . self::BUILD_DIR;
