...
Sobol Image Icons renders an image as a CSS mask instead of as a picture. The shape comes from the file,
...
Sobol Image Icons carries the mask in a CSS custom property, which survives the filtering, and resolves
...
2. Activate it.
3. Add the **Sobol Image Icons** block, put one inside a sentence with the toolbar's icon button, or open
...
The source it was built from is at https://github.com/lsobolew/sobol-image-icons, tagged `v0.1.0` for
this release. The block sources are in `plugins/sobol-image-icons/blocks/`, and the build is reproduced
with:

`npm ci && npm run build` in `plugins/sobol-image-icons/`
...
No, and that is rather the point. Sobol Image Icons works with what is already in your media library, so
