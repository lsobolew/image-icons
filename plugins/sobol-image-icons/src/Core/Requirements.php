...
final class Requirements {
	...
	public static function met(): bool {
		if ( version_compare( PHP_VERSION, SOBOL_IMAGE_ICONS_MIN_PHP, '<' ) ) {
			self::$failed = 'php';

			return false;
		}

		if ( version_compare( (string) get_bloginfo( 'version' ), SOBOL_IMAGE_ICONS_MIN_WP, '<' ) ) {
			self::$failed = 'wp';
	...
	public static function render_notice(): void {
		...
		if ( 'php' === self::$failed ) {
			$message = sprintf(
				...
				__( 'Sobol Image Icons requires PHP %1$s or newer. This server runs %2$s.', 'sobol-image-icons' ),
				SOBOL_IMAGE_ICONS_MIN_PHP,
				PHP_VERSION
			);
		} else {
			$message = sprintf(
				/* translators: 1: required WordPress version, 2: WordPress version this site runs */
				__( 'Sobol Image Icons requires WordPress %1$s or newer. This site runs %2$s.', 'sobol-image-icons' ),
				SOBOL_IMAGE_ICONS_MIN_WP,
