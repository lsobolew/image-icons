...
final class Module implements ModuleContract {
	...
	public function pass_mask_formats(): void {
		$handle = generate_block_asset_handle( 'sobol-image-icons/icon', 'editorScript' );
		...
		wp_add_inline_script(
			$handle,
			'window.sobolImageIcons = window.sobolImageIcons || {}; window.sobolImageIcons.maskFormats = '
	 ...
	 * button - or a paragraph with an inline icon - and no Sobol Image Icons block would get the markup
	 * without the CSS.
	 *
	...
	private function enqueue_host_block_styles(): void {
		...
		$path     = SOBOL_IMAGE_ICONS_DIR . $relative;
		...
		foreach ( self::HOST_BLOCKS as $block ) {
			wp_enqueue_block_style(
				$block,
				array(
					'handle' => 'sobol-image-icons-inline',
					'src'    => SOBOL_IMAGE_ICONS_URL . $relative,
					'path'   => $path,
	...
	public function block_directories(): array {
		$build = SOBOL_IMAGE_ICONS_DIR . self::BUILD_DIR;
