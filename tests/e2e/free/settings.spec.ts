...
test.describe( 'Settings screen', () => {
	test( 'is reachable from the Settings menu', async ( { admin, page } ) => {
		...
		await expect(
			page.getByRole( 'heading', { name: 'Masked Icon', level: 1 } )
		).toBeVisible();
	} );
	...
	test( 'saves changed values', async ( { admin, page } ) => {
		...
		await page.fill( 'input[name="masked_icon_settings[api_label]"]', label );
		await page.fill( 'input[name="masked_icon_settings[items_per_page]"]', '7' );
		...
		await expect(
			page.locator( 'input[name="masked_icon_settings[items_per_page]"]' )
		).toHaveValue( '7' );
	} );
	...
	test( 'rejects out-of-range values', async ( { admin, page } ) => {
		...
		await page.evaluate( () => {
			const input = document.querySelector(
				'input[name="masked_icon_settings[items_per_page]"]'
			);
