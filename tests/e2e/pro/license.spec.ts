...
test.describe( 'Pro edition', () => {
	test( 'adds its licence section to the free plugin settings screen', async ( {
		...
		await admin.visitAdminPage( 'options-general.php', 'page=masked-icon' );

		await expect( page.getByRole( 'heading', { name: 'Pro licence' } ) ).toBeVisible();
		await expect( page.locator( '#masked-icon-pro-license-key' ) ).toBeVisible();
		await expect( page.locator( '#masked-icon-pro-license-status' ) ).toContainText( 'no key' );
	} );
	...
	test( 'extends the free plugin REST response', async ( { requestUtils } ) => {
		...
		await requestUtils.rest( {
			method: 'DELETE',
			path: `/wp/v2/maskedicon_item/${ item.id }`,
			params: { force: true },
		} );
	} );
	...
	test( 'saves the licence key in the same form as the rest of the settings', async ( {
		...
		await expect( page.locator( '#masked-icon-settings form' ) ).toHaveCount( 1 );

		await page.fill( 'input[name="masked_icon_pro_license[key]"]', 'TEST-KEY-0000' );
		await page.fill( 'input[name="masked_icon_settings[api_label]"]', 'saved-with-licence' );
		...
		await expect( page.locator( '#masked-icon-pro-license-key' ) ).toHaveValue( 'TEST-KEY-0000' );
		await expect( page.locator( '#masked-icon-pro-license-status' ) ).toContainText( 'unknown' );
		await expect(
			page.locator( 'input[name="masked_icon_settings[api_label]"]' )
		...
		await page.fill( 'input[name="masked_icon_pro_license[key]"]', '' );
		await page.getByRole( 'button', { name: SAVE_BUTTON } ).click();
		await expect( page.locator( '#setting-error-settings_updated' ) ).toBeVisible();
		await expect( page.locator( '#masked-icon-pro-license-status' ) ).toContainText( 'no key' );
	} );
